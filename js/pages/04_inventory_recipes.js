...
/* =====================================================
   STOCK PAGE 04 : RECIPES — REBUILT
   ===================================================== */

POS.pages.inventoryRecipes = async function(){

  /*
   * เหมือน Stock Page 05 : Movement
   * คืน HTML ก่อน แล้วค่อย INIT หลัง DOM ถูกสร้าง
   */
  setTimeout(function(){
  ...
  return `
    <div class="inventory-subpage">
      ...
      ">
        <div>
          <h1 class="page-title" style="margin:0 0 5px;font-size:30px;font-weight:800;color:#1f2937;">
            🍳 Recipes
          </h1>
        ...
        <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;">
          <button
            ...
            style="min-height:42px;padding:0 16px;border-radius:10px;font-weight:700;"
          >← กลับหน้าสต็อก</button>
          ...
          <button
            ...
            style="min-height:42px;padding:0 18px;border-radius:10px;font-weight:700;background:#e8f6ec;color:#267a3d;border:1px solid #b9dec3;"
          >➕ เพิ่มสูตร</button>
        </div>
      </div>
      ...
      <div style="display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:16px;margin-bottom:20px;">
        <div class="card" style="margin:0;min-height:100px;box-sizing:border-box;">
          <div style="color:#94a3b8;font-size:13px;font-weight:700;">สูตรทั้งหมด</div>
          <div id="posInventoryRecipesTotal" style="margin-top:7px;font-size:25px;font-weight:800;color:#1f2937;">0</div>
          <div style="margin-top:3px;color:#94a3b8;font-size:12px;">รายการ</div>
        </div>
        <div class="card" style="margin:0;min-height:100px;box-sizing:border-box;">
          <div style="color:#94a3b8;font-size:13px;font-weight:700;">ใช้งานอยู่</div>
          <div id="posInventoryRecipesActive" style="margin-top:7px;font-size:25px;font-weight:800;color:#267a3d;">0</div>
          <div style="margin-top:3px;color:#94a3b8;font-size:12px;">สูตร</div>
        </div>
        <div class="card" style="margin:0;min-height:100px;box-sizing:border-box;">
          <div style="color:#94a3b8;font-size:13px;font-weight:700;">วัตถุดิบที่ใช้</div>
          <div id="posInventoryRecipesIngredients" style="margin-top:7px;font-size:25px;font-weight:800;color:#2563eb;">0</div>
          <div style="margin-top:3px;color:#94a3b8;font-size:12px;">รายการ</div>
        </div>
      </div>
      ...
      <div class="card" style="margin-bottom:20px;">
        <div style="display:grid;grid-template-columns:minmax(0,1fr) auto;gap:12px;align-items:end;">
          <div>
            <label style="display:block;margin-bottom:7px;color:#475569;font-size:13px;font-weight:700;">🔎 ค้นหาสูตร</label>
            <input
              id="posInventoryRecipesSearch"
              type="text"
              placeholder="ค้นหาชื่อสูตร / รหัสสูตร"
              style="width:100%;height:43px;box-sizing:border-box;padding:0 13px;border:1px solid #d7dee8;border-radius:9px;outline:none;font-size:14px;"
            >
          </div>
          <button
            ...
            class="btn-secondary"
            type="button"
            style="min-height:43px;padding:0 15px;border-radius:9px;font-weight:700;"
          >🔄 รีเฟรช</button>
        </div>
      </div>

      <!-- LIST — same simple structure as Movement -->
      <div class="card" style="padding:0;overflow:hidden;">
        <div style="display:flex;justify-content:space-between;align-items:center;gap:12px;padding:18px 20px;border-bottom:1px solid #eef1f4;">
          <div>
            <div style="font-size:18px;font-weight:800;color:#1f2937;">📋 รายการสูตร</div>
            <div style="margin-top:4px;color:#94a3b8;font-size:12px;">จัดการสูตรและวัตถุดิบที่ใช้ในแต่ละเมนู</div>
          </div>
          <div id="posInventoryRecipesListCount" style="padding:7px 11px;border-radius:999px;background:#f8fafc;color:#64748b;font-size:12px;font-weight:700;white-space:nowrap;">0 รายการ</div>
        </div>
          ...
          <table style="width:100%;border-collapse:collapse;min-width:980px;">
            <thead>
              <tr style="background:#f8fafc;border-bottom:1px solid #e2e8f0;">
                <th style="padding:13px 12px;text-align:left;white-space:nowrap;color:#64748b;font-size:12px;font-weight:800;">รหัสสูตร</th>
                <th style="padding:13px 12px;text-align:left;white-space:nowrap;color:#64748b;font-size:12px;font-weight:800;">ชื่อสูตร</th>
                <th style="padding:13px 12px;text-align:center;white-space:nowrap;color:#64748b;font-size:12px;font-weight:800;">จำนวนวัตถุดิบ</th>
                <th style="padding:13px 12px;text-align:center;white-space:nowrap;color:#64748b;font-size:12px;font-weight:800;">สถานะ</th>
                <th style="padding:13px 12px;text-align:center;white-space:nowrap;color:#64748b;font-size:12px;font-weight:800;">จัดการ</th>
              </tr>
            </thead>
            <tbody id="posInventoryRecipesTableBody">
              <tr>
                <td colspan="5" style="padding:55px 20px;text-align:center;color:#94a3b8;">กำลังโหลดข้อมูลสูตร...</td>
              </tr>
            </tbody>
          </table>
        </div>
...
/* =====================================================
   RECIPES : RENDER — REBUILT
   ===================================================== */

POS.inventoryRecipesRender = function(){

  const body = document.getElementById("posInventoryRecipesTableBody");
  if(!body) return;

  const searchInput = document.getElementById("posInventoryRecipesSearch");
  const keyword = String(searchInput?.value || "").trim().toLowerCase();
  const recipes = Array.isArray(POS.inventoryRecipesData) ? POS.inventoryRecipesData : [];
  ...
  for(const item of recipes){
    ...
    let group = grouped.get(menuId);
    if(!group){
      group = {
        menu_id: menuId,
        menu_sku: item?.menu_sku || "-",
        menu_name: item?.menu_name || "-",
        items: []
      };
      grouped.set(menuId, group);
    }
  ...
  let rows = allGroups;
  if(keyword){
    rows = allGroups.filter(function(group){
      return (
  ...
  rows.sort(function(a,b){
    return String(a.menu_name || "").localeCompare(String(b.menu_name || ""), "th");
  });

  const totalEl = document.getElementById("posInventoryRecipesTotal");
  const activeEl = document.getElementById("posInventoryRecipesActive");
  const ingredientEl = document.getElementById("posInventoryRecipesIngredients");
  const listCountEl = document.getElementById("posInventoryRecipesListCount");

  if(totalEl) totalEl.textContent = String(rows.length);
  if(activeEl) activeEl.textContent = String(allGroups.length);
  if(ingredientEl){
    ingredientEl.textContent = String(
      new Set(recipes.map(function(x){ return String(x?.ingredient_id || ""); }).filter(Boolean)).size
    );
  }
  if(listCountEl) listCountEl.textContent = String(rows.length) + " รายการ";
      ...
      <tr>
        <td colspan="5" style="padding:60px 20px;text-align:center;">
          ...
          <div style="font-size:15px;font-weight:800;color:#64748b;">
            ${keyword ? "ไม่พบสูตรที่ค้นหา" : "ยังไม่มีข้อมูลสูตร"}
          </div>
          <div style="margin-top:5px;font-size:13px;color:#94a3b8;">
            ${keyword ? "ลองเปลี่ยนคำค้นหา" : "กด “เพิ่มสูตร” เพื่อเริ่มสร้างสูตรอาหาร"}
          </div>
        </td>
      </tr>
    `;
  ...
  for(let i = 0; i < rows.length; i++){
    const group = rows[i];
    const menuId = POS.inventoryRecipesEscape(group.menu_id);

    html[i] = `
      <tr style="border-bottom:1px solid #eef1f4;">
        <td style="padding:13px 12px;font-size:13px;white-space:nowrap;color:#64748b;font-weight:700;">
          ${POS.inventoryRecipesEscape(group.menu_sku)}
        </td>
        <td style="padding:13px 12px;font-size:14px;white-space:nowrap;color:#1f2937;font-weight:800;">
          ${POS.inventoryRecipesEscape(group.menu_name)}
        </td>
        <td style="padding:13px 12px;text-align:center;white-space:nowrap;font-size:14px;font-weight:800;color:#2563eb;">
          ${group.items.length} รายการ
        </td>
        <td style="padding:13px 12px;text-align:center;white-space:nowrap;">
          <span style="display:inline-block;min-width:72px;padding:6px 10px;border-radius:999px;background:#e8f6ec;color:#267a3d;font-size:12px;font-weight:800;">ใช้งาน</span>
        </td>
        <td style="padding:13px 12px;text-align:center;white-space:nowrap;">
          <button type="button" data-recipe-menu-id="${menuId}" style="min-height:36px;padding:0 12px;border-radius:8px;border:1px solid #d7dee8;background:#fff;color:#334155;font-weight:700;cursor:pointer;">
            ⚙️ จัดการ
          </button>
        </td>
      </tr>
...
/* =====================================================
   RECIPES : INIT — REBUILT
   ===================================================== */

POS.inventoryRecipesInit = function(){

  const addButton = document.getElementById("posInventoryRecipesAddBtn");
  const refreshButton = document.getElementById("posInventoryRecipesRefreshBtn");
  const searchInput = document.getElementById("posInventoryRecipesSearch");
  const tableBody = document.getElementById("posInventoryRecipesTableBody");
  ...
  if(tableBody && !tableBody.dataset.recipeClickBound){
    ...
    tableBody.addEventListener("click", function(event){
      const button = event.target.closest("button[data-recipe-menu-id]");
      if(!button || !tableBody.contains(button)) return;
      POS.inventoryRecipesOpenManage(button.getAttribute("data-recipe-menu-id") || "");
    });
  }

  /* Router เป็นผู้ LOAD ตอนเปิดหน้า; refresh ยัง LOAD ได้ตามปกติ */
